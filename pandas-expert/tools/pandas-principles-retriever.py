"""
Tool Wrapper: return the most relevant bullets from references/core-pandas-principles.md.
"""
...
def _load_principle_bullets() -> List[str]:
    path = _skill_root() / "references" / "core-pandas-principles.md"
...
def retrieve_principles(query: str, max_results: int = 7) -> List[str]:
    """Return up to max_results pandas principle lines ranked by keyword overlap."""
...
tool_spec = {
    "name": "pandas-principles-retriever",
    "description": "Retrieves only relevant pandas data manipulation and optimization principles by query keywords.",
    "parameters": {
        "query": {"type": "string"},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Retrieve pandas principles.")
