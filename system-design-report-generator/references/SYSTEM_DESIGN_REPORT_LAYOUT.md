...
Agents **`pin_map` from a cue** with `session=<catalog from AGENT-CONTEXT.md>` (leftover `anchor=` named leftover) before opening section files. Full procedure: [memnet-report-pipeline.md](memnet-report-pipeline.md).
