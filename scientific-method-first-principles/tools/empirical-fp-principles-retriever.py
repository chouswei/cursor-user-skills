"""
Tool Wrapper: return the most relevant bullets from references/core-empirical-fp-principles.md.

ADK: register as tool name empirical-fp-principles-retriever with parameters query (str), max_results (int).
"""
...
def _load_principle_bullets() -> List[str]:
    path = _skill_root() / "references" / "core-empirical-fp-principles.md"
...
tool_spec = {
    "name": "empirical-fp-principles-retriever",
    "description": (
        "Retrieves empirical + first-principles fusion rules from core-empirical-fp-principles.md "
        "by query keywords (max_results)."
    ),
    "parameters": {
        "query": {"type": "string"},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Retrieve empirical + first-principles bullets.")
