...
**Wire reminder:** pin_map = **shaped subgraph** from a cue. Product write = **`mutate`** (CREATE / MATCH-SET / DELETE). leftover `id:'NEW'` / leftover `add` named leftover.
...
- **Handoff aid:** `memnet-goldfish-loop.mdc` + `memnet-use` + `memnet-format/SKILL.md` + `mcp-memnet` + `memnet-nested-sessions` + `memnet-multitask` (Multitask / Task sub-agents) + `sysml-gql` + `sysml-memnet-pipeline.md`; plain Markdown when MemNet down.
...
(Copy nicknames from the pin map after Commit. leftover `id:'NEW'` mint is leftover.)
