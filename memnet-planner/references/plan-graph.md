...
`s1` and `s2` share `wave: 1` (disjoint `scope`). `s3` is `wave: 2` after both. Waves: [execution-waves.md](execution-waves.md). After Bind ready, each step is one atom (one path, qname, or proof) with a required role -- do not mint one step that bundles several files for a single worker.
