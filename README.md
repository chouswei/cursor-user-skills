...
- MemNet agent I/O: **GQL wire** only (`memnet-use` + `memnet-format` + `mcp-memnet`; cue `pin_map` + product **`mutate`**). Package **`memnet-llm==0.19.0`**. Do not teach leftover `id:'NEW'` / leftover `anchor=` / pipe `@TAG` / Layer as product law. Nested interiors: `memnet-nested-sessions`. SysML bridge: `sysml-gql`.
