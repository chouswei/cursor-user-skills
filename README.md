...
- MemNet agent I/O: **GQL wire** only (`memnet-use` hub, then `memnet-format` + `mcp-memnet`; cue `pin_map` + `mutate`). **Package and PyPI 0.19.2** (`pip install memnet-llm` or `==0.19.2`). Do not teach pipe `@TAG` rows or Layer as agent format. SysML bridge: `sysml-gql`.
