...
- MemNet agent I/O: **GQL wire** only (`memnet-use` hub, then `memnet-format` + `mcp-memnet`; cue `pin_map` + `mutate`). **Package and PyPI 0.19.5** (`pip install memnet-llm` or `==0.19.5`; honesty `c` on 0.19, not usage-method `b`). Do not teach pipe `@TAG` rows or Layer as agent format. SysML bridge: `sysml-gql`.
- Dead product skills (TOON/TRON, novel-writer, merged SysML stubs) were **removed** from the pack; do not reintroduce.
- Deprecate stubs (short `SKILL.md` -> survivor): `sysml-eagle-netlist-parser-tool`, `hardware-custom-pcba-workflow`. Follow the survivor they name.
- Sub-agents: `rules/sub-agent-policy.mdc` (compose). User Rules async (asynchronous) checkpoint pipeline: spawn a wave of disjoint workers, checkpoint, repeat. Execute is atomised parallel workers. Slugs must be on the live Task allowlist; Plan is not the default worker; never `*-fast`.
