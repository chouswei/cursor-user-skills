...
- MemNet agent I/O: **GQL wire** only (`memnet-use` hub, then `memnet-format` + `mcp-memnet`; cue `pin_map` + `mutate`). **Package 0.19.1**; PyPI wheel still **memnet-llm==0.19.0** until twine. Do not teach pipe `@TAG` rows or Layer as agent format. SysML bridge: `sysml-gql`.
