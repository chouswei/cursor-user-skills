...
- Sub-agents: role->slug in `rules/sub-agent-policy.mdc` (unclear/thinking -> `cursor-grok-4.5-low` never FAST; visual items review -> `kimi-k3-max`; clear-step routines -> `composer-2.5`; never `*-fast`).
