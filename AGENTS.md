...
| [analytical-mechanics-propose](analytical-mechanics-propose/SKILL.md) | General analytical-mechanics method transfer (STM thesis is the worked example) |
...
| Open-repo **AGENTS.md** (`modelbasedPrj-*` or `SysMLEdgePrj-*`) | SysML / PCBA when that system repo is open |

---

## Key Rule: Limited Iteration

Do not walk skill folders or `related_skills.txt` as a checklist. Match repo TRG then pack TRG (<=2 passes), open one skill, execute. Ask when routing stays ambiguous.
