...
Product **`memnet-llm` 0.19.0**. PyPI wheel is **0.19.0**. Tools return JSON text. Arg **`session`** (not `session_id`). SSOT: MemNet `parts/memnet-mcp/software/memnet_mcp/server.py`.

This pack: Cursor HTTP **`http://10.0.0.10:18766/mcp`** (`memnet-pi`); TCP store **`:18765`**.
...
leftover façades (registered, not TARGET): `add`, `update`, `query_warm`, `query_walk`. **No** `read_get`. leftover `id_policy` on import is leftover.
