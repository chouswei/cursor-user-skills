...
1. **Package and PyPI 0.19.2** (tag `v0.19.2`; extras 0.10-0.19 unchanged). **Install:** `pip install 'memnet-llm[mcp]'` or `pip install 'memnet-llm[mcp]==0.19.2'`. Optional `[neo4j]` (live claimed 0.14; drivers only). **1.0** unclaimed.
