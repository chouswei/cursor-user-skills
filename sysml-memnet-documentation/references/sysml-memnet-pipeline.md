# SysML + MemNet — pipeline handoffs (wire format)

**Audience:** LLM agents. Structured state **between pipeline steps** uses **MemNet wire** (`@TAG:` pipe rows) when `serve_status` is true — not TOON/TRON blocks in chat.

Pair with [sysml-memnet-snap.md](sysml-memnet-snap.md) (6-step turn), [memnet-report-pipeline.md](../../system-design-report-generator/references/memnet-report-pipeline.md), [memnet-format](../../memnet-format/SKILL.md).
...
## SysML modeling — 6-step wire template

Copy pattern; replace ids and payloads. **Write after each step** (or batch s1–s2, then s3, then s4–s6).
...
**Step 6 skip** (comment-only): `s6:skip` + `update` turn `settled`.

**Resume next turn:** `query_warm(TSK_model_<short>)` — read latest `TSK_turn_*` with `status=in_progress` or last `C_s*` rows; **do not** re-derive from chat.

---

## Report pipeline — G / M steps (wire)
...
Link report atoms: `@EDG` `C_M5` → `mentions` → `@CON_*` / `@PRT_*` (same batch as model delta).
...
**Parent:** `TSK_diagram_<figureId>` `childOf` → `TSK_model_<short>` · `documents` → `@SEC` · `delete_on_settle` until `p6:settle`.

**Pipe codes** (`@CLM` type=`pipe`, `sec` = `TSK_diagram_*`):

| Code | Action | Touches `.md`? |
|------|--------|----------------|
| `p0:types` | `typedBy` histogram → intent bucket | No |
| `p1:scope` | `figure_includes` / `figure_uses` EDG batch | No |
| `p2:place` | Ranks, anchor, `adjacent_to`, `@CLM` stat `deg rank lane declare#` | No |
...
```
step N:   query_warm(TSK_model_* OR TSK_turn_*)
          → act
          → add/update @CLM pipe row for step N
...
| `@CLM.type` | Use |
|-------------|-----|
| `pipe` | Pipeline step outcome (`code` = `sN:…` or `GN:…` / `MN:…`) |
...
- [ ] Next step starts with `query_warm`, not chat memory
