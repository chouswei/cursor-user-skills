...
User-pack store: key **`memnet`** (namespace **`user-memnet`**); URL in `mcp.json` only. InvenTree MCP is not MemNet. MUST NOT treat old `memnet-pi` / `user-memnet-pi` as live. MUST NOT cite `:18766` / `:18765` / `10.0.0.10` or a SKILL.md URL as the live tip.
