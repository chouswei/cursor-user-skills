...
(:SKL {id: 'memnet-use'})-[:COMPLEMENTS {id: 'E_mn_07', note: 'stm_mechanics', recycle: 'persistent'}]->(:SKL {id: 'llm-stm-analytical-mechanics'})
(:SKL {id: 'llm-stm-analytical-mechanics'})-[:COMPLEMENTS {id: 'E_mn_08', note: 'tools', recycle: 'persistent'}]->(:SKL {id: 'mcp-memnet'})
(:SKL {id: 'llm-stm-analytical-mechanics'})-[:COMPLEMENTS {id: 'E_mn_09', note: 'wire', recycle: 'persistent'}]->(:SKL {id: 'memnet-format'})
(:SKL {id: 'llm-stm-analytical-mechanics'})-[:COMPLEMENTS {id: 'E_mn_10', note: 'playbook_pointer', recycle: 'persistent'}]->(:SKL {id: 'memnet-stm-harness'})
(:SKL {id: 'analytical-mechanics-propose'})-[:COMPLEMENTS {id: 'E_am_01', note: 'stm_specialist', recycle: 'persistent'}]->(:SKL {id: 'llm-stm-analytical-mechanics'})
(:SKL {id: 'analytical-mechanics-propose'})-[:SPECIALIZES {id: 'E_am_02', note: 'stm_worked_example', recycle: 'persistent'}]->(:SKL {id: 'llm-stm-analytical-mechanics'})
```

Load `memnet-use` when the job is **using** MemNet. Load `memnet-planner` when a plan must live in the session graph and be updated or repolished. Load `memnet-nested-sessions` when a nest is cut across sessions. Load `memnet-multitask` when Multitask Mode or Task sub-agents are in play (spawn a wave, checkpoint, repeat). Load `analytical-mechanics-propose` when proposing or reviewing an analytical-mechanics framing for any domain (STM thesis is the worked example, not the only target). Load `llm-stm-analytical-mechanics` when wiring or triaging STM from thesis locks (W vs S, ShapeWalk harness, gauge/caps). Load `memnet-stm-harness` when you only need which playbook file to fetch. Load `sysml-gql` when SysML modeling uses MemNet GQL working memory. Ops: MemNet `docs/operations/multi-agent-sessions.md`. Shape: `docs/SHAPE.md`. Version map: `docs/ROADMAP.md` (**package and PyPI 0.19.3**). System-repo pattern: MemNet `docs/application-notes/system/llm-system-dev-multitask.md`.
