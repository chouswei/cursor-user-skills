...
**Package 0.19.1** (Hatch; tag `v0.19.1`; extras 0.10-0.19 unchanged). **PyPI wheel** still **`memnet-llm==0.19.0`** until twine. Cue then `pin_map`; `find` if ego unknown. Empty q is 0.11 outline. Product write is **`mutate`**. leftover `--anchor` / `id:'NEW'` / leftover `add` named leftover. **1.0** unclaimed.
