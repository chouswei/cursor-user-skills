...
| Ask | Skill |
|-----|--------|
| Train / evaluate an NN whose outputs must satisfy equality/inequality physics constraints (projection at inference) | [physics-enforce-constrained-nn](../physics-enforce-constrained-nn/SKILL.md) |
| Embed a **trained ReLU MLP** into MILP (Pyomo / Gurobi) for global opt / verification | [physics-relu-milp-embed](../physics-relu-milp-embed/SKILL.md) |
| Deterministic global opt over a **trained KAN** (MINLP) or compare to ReLU+OMLT | [physics-kan-global-opt](../physics-kan-global-opt/SKILL.md) |
| Propose Lagrangian / Hamiltonian framing before any surrogate | [analytical-mechanics-propose](../analytical-mechanics-propose/SKILL.md) |

## Defaults

- Constraints and locks come from **named physics** (e.g. Beer A = epsilon c ell with locked ell). Never invent assay coefficients, wavelengths, or volumes to make a fit look good.
