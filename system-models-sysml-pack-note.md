...
- **SysML:** all **`sysml-*`**, **`mcp-sysml-v2`**, and **`traceability-footprint-to-sysml`** (open-issue; pack SSOT in that skill’s `references/` — ignore stale `../../../docs` paths). The former **`mcp-sysmledgraph`** folder is retained only as abandoned historical/schema documentation.
