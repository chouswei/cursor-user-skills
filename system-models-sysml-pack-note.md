...
- **SysML:** all **`sysml-*`**, **`sysmledge-workflow`**, **`mcp-sysml-v2`**, and **`traceability-footprint-to-sysml`** (open-issue; pack SSOT in that skill's `references/` -- ignore stale `../../../docs` paths). **`mcp-sysmledgraph`** retargets live graph work to MemNet/SysMLEdge MCP (never Kuzu); historical Kuzu names are refuse-only.
