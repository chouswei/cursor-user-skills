...
Embed a **trained ReLU MLP** as a mixed-integer linear program so optimisation / verification can run over the network with MIP solvers (Pyomo or Gurobi), following [ReLU_ANN_MILP](https://github.com/process-intelligence-research/ReLU_ANN_MILP) / `reluMIP` (Grimstad and Andersson; Schweidtmann lineage).
...
- Net still being trained for constraint satisfaction only -- prefer [physics-enforce-constrained-nn](../physics-enforce-constrained-nn/SKILL.md) first.
...
3. **Couple** MIP vars to outer physics constraints (locked path length, balances) -- physics stays outside the net when exact.
...
- OMLT big-M path appears in the KAN repo MLP baseline -- acceptable sibling tooling when documented.
- Soft alternative for KANs: [physics-kan-global-opt](../physics-kan-global-opt/SKILL.md).
