...
## Repo outputs (`outputs/**/*.md`)

- **Edge labels** (`-->|text|`): one line, ASCII only — no `\n`, no `→` inside the label. See [edge-label-parser-safety.md](../references/edge-label-parser-safety.md).
- **Node labels** may use `\n` (max ~3 lines); put SysML bind detail in caption/legend.
- Validate: `mmdc -i section.md`
