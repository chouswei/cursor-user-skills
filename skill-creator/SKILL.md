...
pipeline_steps:
  ...
  7. Selector snippets (conditional)
     - If wants_selector_update: append GQL `CREATE` rows to `reasoning-strategy-selector/references/skill-graph-seed.wire` (one SKL + >=2 TRG + >=1 typed edge per skill-graph.md density contract); run `python tools/bootstrap_skill_graph.py --regenerate-views`; update `related_skills.txt` if router subset changes then `python tools/sync_related_skills_from_txt.py --write`; `python tools/validate_selector_pack.py --check-views`.
