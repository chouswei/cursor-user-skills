"""
Tool Wrapper: return the most relevant bullets from references/core-skill-patterns.md.

ADK: register as tool name skill-pattern-retriever with parameters query (str), max_results (int).
"""
...
def _load_principle_bullets() -> List[str]:
    path = _skill_root() / "references" / "core-skill-patterns.md"
...
tool_spec = {
    "name": "skill-pattern-retriever",
    "description": "Retrieves skill-design first principles and pattern selection hints by query keywords.",
    "parameters": {
        "query": {"type": "string"},
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Retrieve skill pattern principles.")
